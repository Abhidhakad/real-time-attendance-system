...
export const getTeamRequests = async (managerId, options) => {
...
export const getAllRequests = async (query, options) => {
  const queryObj = {};
...
export const approveRequest = async (requestId, approverId, approverRole, remarks = null) => {
...
export const rejectRequest = async (requestId, approverId, approverRole, remarks = null) => {
  const request = await overtimeRepository.findById(requestId);
  if (!request) {
    const error = new Error('Overtime request not found');
    error.statusCode = 404;
